...
function getAllLinks() {
  ...
  fs.readdirSync('content/communes').forEach(file => {
    if (file.endsWith('.md')) {
      const voieLyonnaiseNumber = file.match(/\d+/g);

      const filePath = path.join('content/communes', file);
        ...
        if (geojson.type === 'FeatureCollection') {
          for (const feature of geojson.features) {
            if (feature.geometry.type === 'LineString') {
              ...
              const validStatus = ['done', 'wip', 'planned', 'tested', 'postponed', 'unknown', 'variante', 'variante-postponed', 'expected'];
